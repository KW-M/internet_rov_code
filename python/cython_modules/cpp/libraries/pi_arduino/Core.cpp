...
void pinMode(uint8_t pin, uint8_t mode)
{
    ...
    if (mode == INPUT || mode == INPUT_PULLUP || mode == INPUT_PULLDOWN || mode == OUTPUT)
    {
        ...
        *(gpio_map + clk_offset) = 0;
        ...
        { // mode == INPUT or INPUT_PULLUP or INPUT_PULLDOWN
...
void tone(uint8_t pin, uint32_t frequency, unsigned long duration, uint32_t block)
{
    ...
    else
    {
        ...
        // modified by me from: &idToneThread[SOC_GPIO_PINS]
        threadId = &idToneThread[pin];
