...
class Relay_GRPC_Client:
    ...
    # blocking function that will keep connected to relay and wait for events from the relay
    async def start(self, msg_handler: MessageHandler):
    ...
    async def connect(self):
        async with self.getChannel() as chan:
            ...
            async for event in eventStream:
                ...
                if event_type == "msg_recived":
                    ...
                    self.msg_handler.handle_incoming_msg(
                ...
                if event_type == "peer_connected":
                    ...
                    print("PYTHON: Got peerConnected event: " + str(ev) +
                          " | exId: " + str(exchange_id))
                    self.msg_handler.handle_peer_connected_message(
                ...
                if event_type == "peer_disconnected":
                    ...
                    print("PYTHON: Got peerDisconnected event: " + str(ev) +
                          " | exId: " + str(exchange_id))
                    self.msg_handler.handle_peer_disconnected_message(
                        src_peer_id=ev.src_peer_id)
