...
function makeJsonApiRequest(url) {
    return fetch(url).then((response)=>{
        ...
        if (!responseText) throw Error("Got no response from rov");
        try {
            responseObject = JSON.parse(responseText);
        } catch (e) {
            throw Error("Got invalid JSON from rov: " + responseText);
        }
        if (responseObject['status'] != 'ok') throw Error(result['message']);
...
function restartROVServices() {
    if (confirm("Are you sure you want to restart services? - The ROV may appear to stop responding and be on a different ngrok url when done.")) window.open("/uwsgi/restart_services");
}
