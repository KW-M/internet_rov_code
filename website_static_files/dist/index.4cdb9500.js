...
function signal(url, onStream, onError, onClose, onMessage, onDataChannelOpen) {
    if ("WebSocket" in window) {
        ...
        wSocket.onopen = function() {
            /* First we create a peer connection */ var config = {
                "iceServers": [
                    {
                        "urls": [
            ...
            if ('ontrack' in peerConnection) peerConnection.ontrack = function(event) {
                console.log('pc.ontrack', event); // <-- new line
                if (event.track.kind === 'video') onStream(event.streams[0]);
                 // <-- new line
            };
            else peerConnection.onaddstream = function(event) {
            ...
            /* kindly signal the remote peer that we would like to initiate a call */ var request1 = {
                what: "call",
                options: {
                    ...
                    vformat: 30,
                    /* 30=640x480, 30 fps */ trickle_ice: true
                }
            };
            console.log("Sending signalling web socket message: ", request1);
            wSocket.send(JSON.stringify(request1));
        };
        wSocket.onmessage = function(evt) {
            ...
            switch(what){
                case "offer":
                    ...
                    peerConnection.setRemoteDescription(new RTCSessionDescription(JSON.parse(data)), function onRemoteSdpSuccess() {
                        hasRemoteDesc = true;
                        addIceCandidates();
    ...
    } else onError("Sorry, this browser does not support Web Sockets. Please use a newer browser.");
}

//# sourceMappingURL=index.4cdb9500.js.map
