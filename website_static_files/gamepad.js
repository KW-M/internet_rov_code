...
var gamepadSupport = {
    ...
    // Whether weï¿½re requestAnimationFrameing like itï¿½s 1999.
    ticking: false,

    // The canonical list of attached gamepads, without ï¿½holesï¿½ (always
    ...
    init: function () {
        ...
        if (!gamepadSupportAvailable) {
            // It doesnï¿½t seem Gamepad API is available ï¿½ show a message telling
    ...
    startPolling: function () {
        // Donï¿½t accidentally start a second loop, man.
    ...
    scheduleNextTick: function () {
        // Only schedule the next frame if we havenï¿½t decided to stop via
        // stopPolling() before.
        if (gamepadSupport.ticking) {
    ...
    pollStatus: function () {
        ...
        for (var i in gamepadSupport.gamepads) {
            ...
            // Donï¿½t do anything if the current timestamp is the same as previous
            // one, which means that the state of the gamepad hasnï¿½t changed.
            // This is only supported by Chrome right now, so the first check
            // makes sure weï¿½re not doing anything if the timestamps are empty
            // or undefined.
